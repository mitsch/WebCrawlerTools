module Main where
	...
	main = do
		args <- getArgs
		document <- openURL "http://de.wikipedia.org/wiki/Liste_der_Universit%C3%A4ten_und_Fachhochschulen_in_%C3%96sterreich" 
		...
		let wikiLinks = map ("http://de.wikipedia.org" ++) (catMaybes $ map getLink (tail (partitions (~== "<tr>") (concat $ partitions (~== "<table class=\"wikitable sortable\">") tags)))) where
			getLink (TagOpen "tr" _ : TagOpen "td" _ : TagOpen "span" _ : TagText _ : TagClose "span" : TagOpen "a" attr  : _) = getAttribute "href" attr
			getLink _ = Nothing
